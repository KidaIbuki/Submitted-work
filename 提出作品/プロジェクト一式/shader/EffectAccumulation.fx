...
float4 ps_main(float2 uv : TEXCOORD0) : COLOR{
    ...
    // ---- 丸み：やや滑らかなノイズ ----
    float smoothNoise = tex2D(fireSamp, float2(angleNorm * 2.5 + time * 0.1, time * 0.15)).r;
    smoothNoise = (smoothNoise - 0.5) * circlePowor;  // ノイズ値を-0.5〜+0.5にして強さを調整
    
    // ---- 尖り：高周波ノイズ ----
    float sharpNoise = tex2D(fireSamp, float2(angleNorm * 10.0 + time * 0.5, time * 0.4)).r;
    sharpNoise = (sharpNoise - 0.5) * sharpnessPowor;

    // ---- 合成：丸み + 尖り ----
    float deform = smoothNoise + sharpNoise;
    dist += deform;
